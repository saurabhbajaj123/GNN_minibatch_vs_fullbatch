...
def main():
    ...
    # data = torch.load(root + "/orkut/orkut_pyg_subgraph.bin")
    ...
    val_idx = data.val_mask.nonzero().view(-1)
    ...
    num_classes = (torch.max(data.y) + 1).item()
    ...
    mp.spawn(
        run,
        args=(world_size, quiver_sampler, quiver_feature, data.y.squeeze(), data.edge_index, split_idx, num_features, num_classes, args),
        nprocs=world_size,
        join=True
    )
