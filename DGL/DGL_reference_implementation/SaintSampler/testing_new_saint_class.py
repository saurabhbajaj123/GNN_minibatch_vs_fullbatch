...
from SaintSamplerClass import SAINTSampler_custom

def _get_data_loader(sampler, device, graph, num_iters, batch_size=1024):
    
    logger.info("Get train data loader")
    train_dataloader = dgl.dataloading.DataLoader(
    # The following arguments are specific to DGL's DataLoader.
    graph,              # The graph
    torch.arange(num_iters),         # The node IDs to iterate over in minibatches
...
def train(graph, dataset, node_features, device, model, args):
    ...
    train_dataloader = _get_data_loader(sampler, device, graph, args.num_iters, args.batch_size)
    ...
    train_time = 0
    # print(f"num iterations = {len(train_nids)/args.batch_size}")
    # print(f"num nodes = {graph.num_nodes()}, num_train = {len(train_nids)}, num_val = {len(valid_nids)}, test = {len(test_nids)}")
    # print(graph.num_nodes(), len(train_nids) + len(valid_nids) + len(test_nids))
    for epoch in range(args.n_epochs):
        ...
        for step, subg in enumerate(train_dataloader):
            ...
            m = subg.ndata["train_mask"].bool()
            # mv = subg.ndata["val_mask"].bool()
            # mt = subg.ndata["test_mask"].bool()
            # print(sum(m) + sum(mv) + sum(mt) == len(labels)) # True
            # print(len(subg.ndata["_ID"]) == len(labels)) # True
