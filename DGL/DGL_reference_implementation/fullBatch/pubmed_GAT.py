...
def train():
    ...
    train_mask = torch.where(graph.ndata['train_mask'])[0]
    val_mask = torch.where(graph.ndata['val_mask'])[0]
    test_mask = torch.where(graph.ndata['test_mask'])[0]

    
    wandb.init(
        project="GAT-pubmed-fullbatch",
        config={
            "epochs": 500,
            "lr": 1e-3,
            "n_hidden": 131,
...
sweep_configuration = {
    ...
    'parameters': 
    {
        ...
        'n_hidden': {'distribution': 'int_uniform', 'min': 32, 'max': 256},
     }
}
sweep_id = wandb.sweep(sweep=sweep_configuration, project='GAT-pubmed-fullbatch')
